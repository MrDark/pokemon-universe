...
type ICondition interface {
	StartCondition(creature ICreature) bool
	ExecuteCondition(creature ICreature, interval int) bool
	EndCondition(creature ICreature, reason int)
	AddCondition(creature ICreature, condition ICondition)
...
func (c *Condition) StartCondition(_creature ICreature) bool {
	if c.GetTicks() > 0 {
		c.endTime = int64(c.GetTicks()) + PUSYS_TIME()
	}
	return true
...
func (c *Condition) ExecuteCondition(_creature ICreature, _interval int) bool {
	if _interval > 0 {
...
func (c *Condition) EndCondition(_creature ICreature, _reason int) {
	// ...
}

func (c *Condition) AddCondition(_creature ICreature, _condition ICondition) {
