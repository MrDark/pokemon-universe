...
type IOMapDB2 struct {
	rowChan chan mysql.Row
}

func (io *IOMapDB2) LoadMap(_map *Map) error {
	io.rowChan = make(chan mysql.Row)
	
	if err := io.loadMapList(_map); err != nil {
		return err
	}

	// Spawn a row processor, in a different goroutine.
	go io.processRows(_map)
